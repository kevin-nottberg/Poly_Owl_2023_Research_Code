    ...
    while(True):
        ...
        while(True):
            if(input("Run new test? (y | n) ") == "y"):
                coding_rate = input("Coding rate: 5, 6, 7, 8? ")
                ...
                for att in attenuations:
                    input("Set the att to {} and press enter when ready to proceed".format(att));
                    ...
                    for i in range(0, int(packet_number)):
                        sender.flushInput();
                        sender.write(b'\n');
